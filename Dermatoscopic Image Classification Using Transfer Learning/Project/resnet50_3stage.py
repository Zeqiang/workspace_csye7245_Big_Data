...
"""
CNN Transfer Learning Using ResNet50_v1

This module demonstrates the entire training part using ResNet50_v1 structure, 
...
Example:
    To run the train script in background, use command:
        ...
        $ nohup python resnet50_3stage.py &
...
# ===============================================================================================================
# Load resney50 model using tf.slim
# ===============================================================================================================
# load model
...
variables_new_layer = [var for var in tf.global_variables() if 'MyPrediction' in var.name]
...
cross_entropy = tf.nn.softmax_cross_entropy_with_logits(logits=logit_squeeze, labels=y_label)
...
logits_initializers = [var.initializer for var in tf.global_variables() if 'resnet_v1_50/logits/' in var.name]
...
#     graph_writer.add_graph(sess.graph)
    ...
    for e in range(1,4):
        ...
        while idx < num_train:
            ...
            sess.run(train_op_1, feed_dict = {x_in:x_batch, y_label:y_true_batch, isTrain:False, Aug_flag:augFlags})
            idx = kidx
...
#         graph_writer.add_summary(summary, e)
    ...
    for e in range(4,11):
        ...
        while idx < num_train:
            kidx = min(idx + BATCH_SIZE, num_train)
            x_batch, y_true_batch, augFlags = next_batch(idx, kidx, train_csv_shuffle, True)
            sess.run(train_op_2, feed_dict = {x_in:x_batch, y_label:y_true_batch, isTrain:False, Aug_flag:augFlags})
            idx = kidx
...
#         graph_writer.add_summary(summary, e)
    ...
    for e in range(11,16):
        ...
        while idx < num_train:
            kidx = min(idx + BATCH_SIZE, num_train)
            x_batch, y_true_batch, augFlags = next_batch(idx, kidx, train_csv_shuffle, True)
            sess.run(train_op_3, feed_dict = {x_in:x_batch, y_label:y_true_batch, isTrain:False, Aug_flag:augFlags})
            idx = kidx
